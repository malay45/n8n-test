 ...
 *
 * FACEIO's REST API only allows programmatic enrollment from a static photo on Enterprise
 * plans; a standard account's fio.enroll() only accepts a live browser webcam feed, so it
 * cannot ingest an admin-uploaded ID scan at all. This plugin therefore never calls FACEIO
 * during enrollment — it keeps a small encrypted reference portrait server-side and defers
 * the actual FACEIO call to verification time (BG_Verification::verify_against_reference()),
 * where the live captured frame and this stored reference are compared via FACEIO's
 * stateless `face-verify` REST endpoint (server-to-server, no prior enrollment required).
 */
...
// 	const META_REFERENCE_PORTRAIT = 'bg_reference_portrait'; // Encrypted, base64 JPEG.
...
// 	 * then crop to just that region. Images are sent as base64 (never a public URL) since the
// 	 * source is a sensitive government ID photo, not something we want to expose a fetchable
// 	 * link to even briefly.
// 	 *
...
class BG_Enrollment {
	...
	private static function crop_portrait_via_pixlab( $image_path ) {
		...
		*/
		...
		// Option A Implementation: Bypass PixLab completely.
		// We return the raw uploaded image data directly, assuming it is already a clean portrait.
		return $image_data;
		...
		$detect_body  = "--$boundary\r\n";
		$detect_body .= "Content-Disposition: form-data; name=\"file\"; filename=\"upload.jpg\"\r\n";
		$detect_body .= "Content-Type: application/octet-stream\r\n\r\n";
		$detect_body .= $image_data . "\r\n";
		$detect_body .= "--$boundary\r\n";
		$detect_body .= "Content-Disposition: form-data; name=\"key\"\r\n\r\n";
		$detect_body .= $key . "\r\n";
		$detect_body .= "--$boundary--\r\n";
		...
		$crop_body  = "--$boundary\r\n";
		$crop_body .= "Content-Disposition: form-data; name=\"file\"; filename=\"upload.jpg\"\r\n";
		$crop_body .= "Content-Type: application/octet-stream\r\n\r\n";
		$crop_body .= $image_data . "\r\n";
		...
		foreach ( $params as $k => $v ) {
			$crop_body .= "--$boundary\r\n";
			$crop_body .= "Content-Disposition: form-data; name=\"$k\"\r\n\r\n";
			$crop_body .= $v . "\r\n";
		}
		$crop_body .= "--$boundary--\r\n";
