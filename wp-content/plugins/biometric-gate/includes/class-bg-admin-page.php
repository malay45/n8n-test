...
class BG_Admin_Page
{
	...
	public static function render_page()
	{
		...
		<div class="wrap bg-admin-wrap">
			...
			<h2 class="nav-tab-wrapper">
				<a href="<?php echo esc_url(self::tab_url('a')); ?>"
					class="nav-tab <?php echo 'a' === $tab ? 'nav-tab-active' : ''; ?>"><?php esc_html_e('User Directory', 'biometric-gate'); ?></a>
				<a href="<?php echo esc_url(self::tab_url('b')); ?>"
					class="nav-tab <?php echo 'b' === $tab ? 'nav-tab-active' : ''; ?>"><?php esc_html_e('Global Settings', 'biometric-gate'); ?></a>
				<a href="<?php echo esc_url(self::tab_url('c')); ?>"
					class="nav-tab <?php echo 'c' === $tab ? 'nav-tab-active' : ''; ?>"><?php esc_html_e('Live Audit Log', 'biometric-gate'); ?></a>
			</h2>
	...
	private static function render_settings_tab()
	{
		...
		<form method="post">
			...
			<table class="form-table" role="presentation">
				...
				<tr>
					<th scope="row"><label
							for="scan_threshold_secs"><?php esc_html_e('Scan Frequency Threshold (Seconds)', 'biometric-gate'); ?></label>
					</th>
					<td><input type="number" min="1" step="1" name="scan_threshold_secs" id="scan_threshold_secs"
							value="<?php echo esc_attr($settings['scan_threshold_secs']); ?>" class="small-text" /></td>
				</tr>
				<tr>
					<th scope="row"><label
							for="guard_window_secs"><?php esc_html_e('Consecutive Scan Bypass Guard Window (Seconds)', 'biometric-gate'); ?></label>
					</th>
					<td>
						<input type="number" min="1" step="1" name="guard_window_secs" id="guard_window_secs"
							value="<?php echo esc_attr($settings['guard_window_secs']); ?>" class="small-text" />
						<p class="description">
							<?php esc_html_e('Must be strictly less than the Scan Frequency Threshold above.', 'biometric-gate'); ?>
						</p>
					</td>
				</tr>
				<tr>
					<th scope="row"><label
							for="path_rules"><?php esc_html_e('Targeted Script Loading Path Rules', 'biometric-gate'); ?></label>
					</th>
					<td>
						<textarea name="path_rules" id="path_rules" rows="3" class="large-text"
							placeholder="/courses/, /lessons/, /topic/"><?php echo esc_textarea($settings['path_rules']); ?></textarea>
						<p class="description">
							<?php esc_html_e('Comma-separated path fragments. Only pages under these paths are gated.', 'biometric-gate'); ?>
						</p>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<th scope="row"><label
							for="no_camera_message"><?php esc_html_e('No-Camera Block Message', 'biometric-gate'); ?></label>
					</th>
					<td>
						<?php
						wp_editor(
				...
				<tr>
					<th scope="row"><label
							for="pixlab_api_key"><?php esc_html_e('PixLab API Key', 'biometric-gate'); ?></label></th>
					<td>
						<input type="text" name="pixlab_api_key" id="pixlab_api_key"
							value="<?php echo esc_attr(BG_Settings::get_pixlab_api_key()); ?>" class="regular-text" />
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="faceio_app_id"><?php esc_html_e('FACEIO App ID', 'biometric-gate'); ?></label>
					</th>
					<td>
						<input type="text" name="faceio_app_id" id="faceio_app_id"
							value="<?php echo esc_attr(BG_Settings::get_faceio_app_id()); ?>" class="regular-text" />
					</td>
				</tr>
				<tr>
					<th scope="row"><label
							for="faceio_secret_key"><?php esc_html_e('FACEIO Secret Key', 'biometric-gate'); ?></label></th>
					<td>
						<input type="password" name="faceio_secret_key" id="faceio_secret_key"
							value="<?php echo esc_attr(BG_Settings::get_faceio_secret_key()); ?>" class="regular-text" />
					</td>
				</tr>
			</table>
	...
	public static function render_user_profile_fields($user)
	{
		...
		<table class="form-table">
			<tr>
				<th><label
						for="bg_bypass_biometric"><?php esc_html_e('Bypass Biometric Verification', 'biometric-gate'); ?></label>
				</th>
				<td>
					<label>
						<input type="checkbox" name="bg_enable_biometric" id="bg_enable_biometric" value="1" <?php checked(!BG_Session::is_bypassed($user->ID)); ?> />
						<?php esc_html_e('Enable Biometric Verification for this user.', 'biometric-gate'); ?>
					</label>
				</td>
			</tr>
		</table>
	...
	public static function save_user_profile_fields($user_id)
	{
		...
		if (isset($_POST['bg_enable_biometric'])) {
