...
class BG_Admin_Page {
	...
	public static function maybe_render_missing_config_notice() {
		...
		$required = array(
			'BIOMETRIC_GATE_ENCRYPTION_KEY'   => __( 'used to encrypt stored reference portraits', 'biometric-gate' ),
			'BIOMETRIC_GATE_PIXLAB_API_KEY'   => __( 'used for ID-photo face cropping', 'biometric-gate' ),
			'BIOMETRIC_GATE_FACEIO_APP_ID'    => __( 'used to call FACEIO\'s face-verify REST API', 'biometric-gate' ),
			'BIOMETRIC_GATE_FACEIO_SECRET_KEY' => __( 'used to call FACEIO\'s face-verify REST API', 'biometric-gate' ),
		);

		$missing = array();
		foreach ( $required as $constant => $reason ) {
			if ( ! defined( $constant ) || '' === constant( $constant ) ) {
				$missing[] = "<code>{$constant}</code> ({$reason})";
			}
		}
